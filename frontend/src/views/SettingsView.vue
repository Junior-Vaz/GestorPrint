...
const handleLogoUpload = async (event: Event) => {
  ...
  try {
    ...
    if (logoInput.value) logoInput.value.value = '' // reset input
  }
}
...
<template>
  <div class="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
    <header>
      <h2 class="text-3xl font-black text-slate-900 tracking-tight text-center md:text-left">Configurações da Empresa</h2>
      <p class="text-slate-500 font-medium text-center md:text-left">Personalize os dados que aparecem nos orçamentos e documentos oficiais.</p>
    </header>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <form v-else @submit.prevent="saveSettings" class="p-8 md:p-12 space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
          <!-- Company Name -->
          <div class="space-y-2">
            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nome da Gráfica / Razão Social</label>
            <input 
              v-model="settings.companyName" 
              type="text" 
              class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
          ...
          <div class="space-y-2">
            ...
            <input 
              ...
              type="text" 
              class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-mono"
          ...
          <!-- Email -->
          <div class="space-y-2">
            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Email de Contato</label>
            <input 
          ...
          <div class="md:col-span-2 space-y-2">
            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Endereço Completo</label>
            <input 
        ...
        <div class="pt-8 border-t border-slate-100">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="md:col-span-2 space-y-2">
              <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Servidor SMTP</label>
              <input v-model="settings.smtpHost" type="text" class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all" placeholder="smtp.gmail.com" />
            </div>
            <div class="space-y-2">
              <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Porta</label>
              <input v-model="settings.smtpPort" type="number" class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-mono" />
            </div>
            <div class="space-y-2">
              <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Usuário SMTP</label>
              <input v-model="settings.smtpUser" type="text" class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all" placeholder="exemplo@gmail.com" />
            </div>
            <div class="space-y-2">
              <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Senha SMTP</label>
              <input v-model="settings.smtpPass" type="password" class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all" />
            </div>
        ...
        <div class="pt-8 border-t border-slate-100">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2">
              <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Access Token (Prod/Sandbox)</label>
              <input v-model="settings.mpAccessToken" type="password" class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all font-mono" placeholder="APP_USR-..." />
            </div>
            <div class="space-y-2">
              <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Public Key</label>
              <input v-model="settings.mpPublicKey" type="text" class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all font-mono" placeholder="APP_USR-..." />
            </div>
        ...
        <div class="flex items-center justify-between pt-6 border-t border-slate-100">
          ...
          <button 
            type="submit" 
            :disabled="saving"
            class="bg-indigo-600 hover:bg-indigo-700 text-white px-10 py-4 rounded-2xl font-black shadow-xl shadow-indigo-100 disabled:opacity-50 transition-all transform active:scale-95 flex items-center gap-3"
          >
            <span v-if="saving" class="w-5 h-5 border-2 border-white/50 border-t-white rounded-full animate-spin"></span>
    ...
    <div class="bg-indigo-50 rounded-3xl p-8 border border-indigo-100">
      <h4 class="text-indigo-900 font-black mb-4 flex items-center gap-2 text-sm uppercase tracking-widest">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
        Visualização no PDF
      </h4>
      <div class="bg-white rounded-xl p-6 border border-indigo-200 shadow-sm max-w-md mx-auto pointer-events-none opacity-80">
