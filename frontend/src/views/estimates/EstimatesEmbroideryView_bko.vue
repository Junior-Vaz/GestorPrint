...
// ── 3D Preview state ──────────────────────────────────────────────────────────
const shirtType = ref<'tshirt' | 'polo' | 'tank' | 'hoodie'>('tshirt')
const shirtColor = ref('#ffffff')
const logoUrl = ref<string | null>(null)
const logoInputRef = ref<HTMLInputElement | null>(null)
const isLogoTexture = ref(false)
...
const totalPrice = computed((): number => {
  ...
  const mk  = (form.markup || 0) / 100
  const up = selectedProduct.value?.unitPrice || 0
  const prodMk = selectedProduct.value ? (selectedProduct.value.markup || 0) / 100 : mk
...
// ── Details builder ───────────────────────────────────────────────────────────
const buildDetails = () => {
  const base = { estimateType: TYPE, productName: form.productName, quantity: form.quantity, markup: form.markup, calculationMethod: typeForm.embroideryMethod, productId: typeForm.productId }
  if (typeForm.embroideryMethod === 'base_plus_colors')
...
<template>
  <div :class="showForm ? 'overflow-hidden' : 'p-4 xl:p-6 max-w-screen-2xl mx-auto space-y-6'">
    ...
    <template v-else>
      ...
      <template v-else>
        ...
        <div class="fixed inset-0 z-10 overflow-hidden">
          ...
          <div class="absolute top-4 z-20 flex items-center gap-3 px-4 py-3
                      bg-white/100 backdrop-blur-xl border border-white/40 rounded-2xl shadow-xl"
               style="left: calc(6.5rem + 1rem); width: 18rem;">
          ...
          <div class="absolute z-20 overflow-y-auto  no-scrollbar
                      bg-white/100 backdrop-blur-xl border border-white/40 rounded-2xl shadow-xl"
               style="left: calc(6.5rem + 1rem); top: 4.5rem; bottom: 1rem; width: 18rem;">
            ...
            <div class="px-4 py-4 space-y-3 border-b border-slate-100">
              ...
              <div v-if="typeForm.embroideryMethod !== 'setup_plus_per_piece'">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 block mb-1">Margem de Lucro %</label>
                <input v-model.number="form.markup" type="number" min="0" max="500"
                  class="w-28 bg-slate-50 border border-slate-100 rounded-xl px-3 py-2.5 font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-sm"/>
              </div>
            </div>
            ...
            <div class="px-4 py-4 space-y-3 border-b border-slate-100">
              ...
              <div>
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 block mb-2">Modo de Aplicação</label>
                <div class="flex gap-1.5">
                  <button @click="isLogoTexture = !isLogoTexture; isFullTexture = false"
                    :class="['flex-1 py-2 rounded-xl text-xs font-bold border transition-all', isLogoTexture ? 'bg-pink-500 text-white border-pink-500 shadow-sm' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300']">
                    Logo no Peito
                  </button>
                  <button @click="isFullTexture = !isFullTexture; isLogoTexture = false"
                    :class="['flex-1 py-2 rounded-xl text-xs font-bold border transition-all', isFullTexture ? 'bg-pink-500 text-white border-pink-500 shadow-sm' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300']">
                    Camiseta Toda
                  </button>
                </div>
              </div>
              ...
              <!-- Logo (chest decal) -->
              <div v-if="isLogoTexture">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 block mb-2">Logo / Arte (peito)</label>
                <div class="flex items-center gap-3 flex-wrap">
                  <button @click="logoInputRef?.click()"
                    ...
                    {{ logoUrl ? 'Trocar' : 'Carregar imagem' }}
                  </button>
                  <button v-if="logoUrl" @click="logoUrl = null; isLogoTexture = false" class="text-red-400 hover:text-red-600 text-xs font-bold">Remover</button>
                  <span v-if="logoUrl" class="text-[10px] text-emerald-500 font-bold">✓ Carregada</span>
                </div>
                <input ref="logoInputRef" type="file" accept="image/*" class="hidden" @change="handleLogoUpload"/>
              </div>

              <!-- Full fill decal -->
              <div v-if="isFullTexture">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 block mb-2">Arte Completa (camiseta toda)</label>
                <div class="flex items-center gap-3 flex-wrap">
          ...
          <div class="absolute top-4 right-4 z-20 flex items-center gap-1 px-3 py-2
                      bg-white/80 backdrop-blur-xl border border-white/40 rounded-2xl shadow-xl">
            <button v-for="(label, sType) in { tshirt: 'Camiseta', polo: 'Polo', tank: 'Regata', hoodie: 'Moletom' }" :key="sType"
              @click="shirtType = (sType as any)"
              :class="['px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all', shirtType === sType ? 'bg-pink-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200']">
              {{ label }}
            </button>
          </div>
          ...
          <div class="absolute bottom-4 right-4 z-20 overflow-hidden
            ...
            <div class="px-5 py-3.5 flex items-center justify-between">
              <div>
                <div class="text-pink-500 text-[9px] font-black uppercase tracking-widest mb-0.5">Total do Orçamento</div>
              ...
              <span class="text-2xl font-black text-pink-500 font-mono shrink-0 ml-3">{{ fmtCurrency(totalPrice) }}</span>
            </div>
