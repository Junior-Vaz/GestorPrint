  ...
  <div class="p-6 max-w-7xl mx-auto space-y-6">
    ...
    <div class="bg-white/50 backdrop-blur-md p-6 rounded-2xl border border-white/20 shadow-xl shadow-slate-200/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-3xl font-extrabold text-slate-800 tracking-tight flex items-center gap-3">PDV Balcão</h1>
          <p class="text-slate-500 mt-1 font-medium italic">Venda rápida e controle de caixa</p>
        </div>
      </div>
    ...
    <!-- Two-column layout -->
    <div class="flex flex-col lg:flex-row gap-6 min-h-[calc(100vh-220px)]">

      <!-- Left: Store / Products -->
      <div class="flex-1 flex flex-col bg-white/80 backdrop-blur-xl rounded-3xl border border-white/40 shadow-2xl shadow-slate-200/60 overflow-hidden">

        <!-- Search + Tabs Header -->
        <div class="p-6 border-b border-slate-100/80 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 class="text-lg font-extrabold text-slate-800 tracking-tight">Frente de Caixa</h2>
          ...
          <div class="relative w-full md:w-72">
            ...
            <input v-model="searchQuery" type="text" placeholder="Buscar por nome..." class="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-xl font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-sm">
          </div>
        </div>

        <!-- Category Tabs -->
        <div class="px-6 py-4 border-b border-slate-100/80 bg-slate-50/30 overflow-x-auto flex gap-2 no-scrollbar">
          <button
            @click="activeType = null"
            :class="['px-4 py-2 rounded-xl text-xs font-black transition-all border whitespace-nowrap', activeType === null ? 'bg-slate-800 text-white border-slate-800 shadow-md' : 'border-slate-200 text-slate-500 hover:border-slate-400 bg-white/80']"
          >
            Todos
          </button>
          <button
            ...
            :class="['px-4 py-2 rounded-xl text-xs font-black transition-all border whitespace-nowrap flex items-center gap-2', activeType === type.id ? 'text-white border-transparent shadow-md' : 'border-slate-200 text-slate-500 hover:border-slate-400 bg-white/80']"
        ...
        <div class="flex-1 overflow-y-auto p-6">
          ...
          <div v-else class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4 pb-6">
            <button
              v-for="product in filteredProducts" :key="product.id"
              @click="addToCart(product)"
              class="group text-left bg-white/80 backdrop-blur-xl border border-white/40 p-4 rounded-2xl hover:border-indigo-400 hover:shadow-lg hover:shadow-indigo-900/5 transition-all outline-none focus:ring-2 focus:ring-indigo-500 relative overflow-hidden"
            >
              <!-- Badge -->
      ...
      <!-- Right: Cart & Checkout -->
      <div class="w-full lg:w-[400px] xl:w-[480px] flex flex-col bg-white/80 backdrop-blur-xl rounded-3xl border border-white/40 shadow-2xl shadow-slate-200/60 overflow-hidden shrink-0 relative">

        <!-- Cart Header -->
        <div class="p-6 border-b border-slate-100/80 flex justify-between items-center bg-slate-50/40">
          ...
          <div class="flex items-center gap-3">
            <button @click="showSangriaModal = true" class="w-8 h-8 flex items-center justify-center bg-red-50 text-red-400 hover:bg-red-500 hover:text-white rounded-lg transition-all" title="Retirada de Caixa (Sangria)">
            ...
            <button v-if="cart.length > 0" @click="clearCart" class="text-xs font-black text-slate-400 hover:text-red-500 transition-colors">
              Limpar
            </button>
          </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-3">
          <div v-if="cart.length === 0" class="h-full flex flex-col justify-center items-center text-slate-400 space-y-3">
          ...
          <TransitionGroup name="list">
            <div v-for="item in cart" :key="item.id" class="bg-slate-50 border border-slate-100 p-3 rounded-2xl flex flex-col justify-between group">
              <div class="flex justify-between items-start mb-3">
                <div class="pr-2">
                  <h4 class="font-bold text-sm leading-tight text-slate-800 mb-0.5">{{ item.name }}</h4>
                  <div class="text-[10px] text-slate-400 font-mono">
                    R$ {{ item.unitPrice.toFixed(2) }} / {{ item.unit }}
                  </div>
                </div>
                <div class="text-right shrink-0 font-mono font-black text-indigo-600">
                  R$ {{ (item.unitPrice * item.cartQuantity).toFixed(2) }}
                </div>
              </div>
              <div class="flex justify-between items-center bg-white rounded-xl p-1 border border-slate-100">
                <div class="flex items-center">
                  <button @click="updateQuantity(item, -1)" class="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-all font-bold">-</button>
                  <input
                    type="number"
                    v-model.number="item.cartQuantity"
                    @change="handleManualQuantity(item)"
                    class="w-12 bg-transparent text-center font-black text-sm font-mono border-none focus:ring-0 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none text-slate-800"
                  />
                  <button @click="updateQuantity(item, 1)" class="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-all font-bold">+</button>
                </div>
                <button @click="removeFromCart(item.id)" class="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all">
        ...
        <!-- Checkout Footer -->
        <div class="bg-slate-900 p-6 rounded-t-3xl border-t border-indigo-500/20 z-10">
          <!-- Customer & Discount -->
          <div class="grid grid-cols-2 gap-4 mb-4">
            <div class="relative">
              <label class="text-[10px] font-black text-indigo-300/60 uppercase tracking-widest mb-1.5 block ml-1">Cliente</label>
              <input
            ...
            <div>
              <label class="flex items-center justify-between text-[10px] font-black text-indigo-300/60 uppercase tracking-widest mb-1.5 cursor-pointer ml-1">
                <span>Desconto</span>
              ...
              <div class="space-y-4">
                <div class="space-y-2">
                  ...
                  <input
                    ...
                    required
                    autofocus
                    class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 font-bold text-slate-700 outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all text-2xl text-red-600"
                  />
                </div>
